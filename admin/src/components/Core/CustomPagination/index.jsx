...
class PaginationComponent extends PureComponent {
  ...
  paginationItems = () => {
    ...
    const { firstPageText, previousPageText, nextPageText, lastPageText } =
      this.props;
    ...
    const firstPaginationNumber = this.getFirstPaginationNumber(
      activePage,
      pages
    );
    const lastPaginationNumber = this.getLastPaginationNumber(
  ...
  nextOrPreviousPagItem = (name, page, direction) => {
    ...
    name === "N"
      ? (classIconName = "fa-angle-right")
      : (classIconName = "fa-angle-left");
    return (
      <PaginationItem
        key={name}
  ...
  handleClick = (event) => {
    const newActivePage = parseInt(
      event.currentTarget.getAttribute("id").split("pagebutton").pop(),
      10
    );
